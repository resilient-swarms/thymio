/**
 * @file <argos3/plugins/robots/kheperaiv/control_interface/ci_kheperaiv_proximity_sensor.cpp>
 *
 * @author Carlo Pinciroli <ilpincy@gmail.com>
 */

#include "ci_kheperaiv_proximity_sensor.h"
...
namespace argos {
   ...
   CCI_KheperaIVProximitySensor::CCI_KheperaIVProximitySensor() : m_tReadings(7) 
   {
   ...
   const CCI_KheperaIVProximitySensor::TReadings& CCI_KheperaIVProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_KheperaIVProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_KheperaIVProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_KheperaIVProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_KheperaIVProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
