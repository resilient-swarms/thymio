/**
 * @file <argos3/plugins/robots/Thymio/control_interface/ci_Thymio_proximity_sensor.cpp>
 *
...
//         m_tReadings[i].Angle = i * SPACING;
   ...
   const CCI_ThymioProximitySensor::TReadings& CCI_ThymioProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_ThymioProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_ThymioProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_ThymioProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_ThymioProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
